...
class DataSaver {
  ...
  /// - Throws: An error of type `GrandArtError` if the encoding fails or the data is empty.
  ...
  func saveData() throws -> Data? {
    ...
    do {
      ...
      if data.isEmpty {
        throw GrandArtError.emptyData
      }
      return data
    } catch {
      throw GrandArtError.encodingError
    }
  }
  ...
  func saveToFile(withPanel savePanel: NSSavePanel, completionHandler: @escaping (Result<Void, Error>) -> Void) {
    do {
      guard let data = try saveData() else {
        completionHandler(.failure(GrandArtError.encodingError))
        return
      }
      ...
      savePanel.begin { result in
        if result == .OK {
          ...
          completionHandler(.failure(GrandArtError.failedSaving))
        }
      }
    } catch {
      completionHandler(.failure(error))
